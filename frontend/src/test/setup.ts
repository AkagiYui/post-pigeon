...
// 真实的 iconify-icon 会去 api.iconify.design 拉图标数据，拿到响应后再用 setTimeout
// 延后一轮才把 SVG 写进 document。单测既不该联网，这个回调也常常落在 jsdom 环境销毁
// 之后，抛出 "document is not defined" 的 unhandled error 把整轮测试判成失败（回调是
// 响应到达时才排期的，收尾时清定时器也拦不住）。测试里换成一个不联网的占位元素。
vi.mock("@iconify-icon/solid", () => ({
  Icon: (props: { icon: string, class?: string }) => {
    const el = document.createElement("span")
    createRenderEffect(() => {
      el.dataset.icon = props.icon
