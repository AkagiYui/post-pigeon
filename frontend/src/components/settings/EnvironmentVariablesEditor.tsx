// 环境变量编辑器：可拖拽排序的变量表格，含列宽调节、两步确认删除与秘密变量遮罩。
//
// 它连同四个私有行组件一起占了 ProjectEnvironmentSettings 六成篇幅，
// 与「环境列表 / 模块前置 URL」并无耦合，单独成文件后两边都好读。
...
import { EnvironmentService } from "@/../bindings/PostPigeon/internal/services"
 ...
 * - 修改后显示未保存提示
 */
export function EnvironmentVariablesEditor(props: { ref: EditorSaveRef; environmentId: string }) {
  // 从服务器加载的原始变量（用于脏检测对比）
