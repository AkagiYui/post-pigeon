...
function HomePage() {
  ...
  return (
    <main class="page-shell px-3 pb-10 pt-6 sm:px-6 sm:pb-14 sm:pt-8">
      <section class="glass-panel rise-in relative overflow-hidden rounded-4xl px-5 py-6 sm:px-8 sm:py-8 lg:px-10 lg:py-10">
        ...
        <div class="relative z-10 grid gap-8 lg:grid-cols-[1.25fr_0.95fr] lg:items-start">
          <div>
            ...
            <h1 class="headline-font max-w-3xl text-4xl font-bold tracking-tight text-(--ink) sm:text-5xl lg:text-6xl">
              用 SolidJS 和类型安全路由，重建你的 Wails 前端基础设施。
            </h1>
            <p class="mt-4 max-w-2xl text-sm leading-7 text-(--ink-soft) sm:text-base">
            ...
            <div class="mt-6 grid gap-3 sm:grid-cols-3">
              <For each={features}>
                {(feature, index) => {
                  ...
                  return (
                    <article
                      class="rounded-3xl border border-(--line) bg-white/70 p-4 shadow-[0_12px_24px_rgba(18,32,39,0.05)]"
                      style={{ 'animation-delay': `${index() * 80 + 80}ms` }}
                    >
                      <Icon class="h-5 w-5 text-(--accent-deep)" />
                      <h2 class="mt-3 text-sm font-bold text-(--ink)">{feature.title}</h2>
                      <p class="mt-2 text-sm leading-6 text-(--ink-soft)">{feature.desc}</p>
                    </article>
                  )
                }}
              </For>
          ...
          <section class="glass-panel rounded-[1.75rem] border-white/50 p-5 sm:p-6">
            <div class="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 class="headline-font text-2xl font-bold text-(--ink)">保留原有问候逻辑</h2>
              </div>
              <div class="rounded-2xl border border-(--line) bg-(--accent-fade) p-3 text-(--accent-deep)">
                <Send class="h-5 w-5" />
              </div>
            </div>

            <div class="mt-5 rounded-3xl border border-(--line) bg-(--hero)/80 p-4">
              <p class="text-xs font-bold uppercase tracking-[0.16em] text-(--accent-deep)">
                Greeting Result
              </p>
              <p class="mt-3 min-h-14 text-base leading-7 text-(--ink)">{result()}</p>
            </div>
            ...
            <label class="mt-5 block text-sm font-medium text-(--ink)">
              ...
              <input
                aria-label="input"
                class="mt-2 w-full rounded-2xl border border-(--line) bg-white px-4 py-3 text-base text-(--ink) outline-none transition focus:border-(--accent) focus:ring-4 focus:ring-[rgba(14,165,164,0.12)]"
                type="text"
                autocomplete="off"
                value={name()}
            ...
            <button
              ...
              class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-transparent bg-(--ink) px-4 py-3 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-[#0b151a] disabled:cursor-not-allowed disabled:opacity-60"
              type="button"
              disabled={submitting()}
              onClick={() => {
                void handleGreet()
            ...
            <div class="mt-5 rounded-3xl border border-dashed border-[rgba(14,165,164,0.22)] bg-white/70 p-4">
              <p class="text-xs font-bold uppercase tracking-[0.16em] text-(--warning)">
                Time Event
              </p>
              <p class="mt-2 text-sm leading-6 text-(--ink-soft)">{timeMessage()}</p>
            </div>
          </section>
        </div>
      </section>
      ...
      <section class="mt-6 grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
        <article class="glass-panel rounded-[1.75rem] p-5 sm:p-6">
          ...
          <h2 class="headline-font text-2xl font-bold text-(--ink)">这次重构做了什么</h2>
          <ul class="mt-4 space-y-3 pl-5 text-sm leading-7 text-(--ink-soft) marker:text-(--accent)">
        ...
        <article class="glass-panel rounded-[1.75rem] p-5 sm:p-6">
          ...
          <h2 class="headline-font text-2xl font-bold text-(--ink)">适合后续开发的页面骨架</h2>
          <div class="mt-4 grid gap-3 sm:grid-cols-2">
            <For each={nextSteps}>
              {([title, desc]) => (
                <div class="rounded-3xl border border-(--line) bg-white/70 p-4">
                  <h3 class="text-sm font-bold text-(--ink)">{title}</h3>
                  <p class="mt-2 text-sm leading-6 text-(--ink-soft)">{desc}</p>
                </div>
              )}
            </For>
          </div>
